...
class ResNet(nn.Module):
    def __init__(self, time_emb_channels, config):
        ...
        self.time_layer = nn.Linear(time_emb_channels, config.output_channel * 2)
        self.norm2 = nn.GroupNorm(config.num_groups_norm, config.output_channels)
        self.dropout = nn.Dropout(config.dropout)
        self.conv2 = mlx_zero_module(
            nn.Conv2d(
                config.output_channels,
                config.output_channels,
                kernel_size=3,
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.proj_out = mlx_zero_module(nn.Conv2d(channels, channels, 1))
        if use_attention_ffn:
            self.ffn = nn.Sequential(
                nn.GroupNorm(32, channels),
                nn.Conv2d(channels, 4 * channels, 1),
                nn.GELU(),
                mlx_zero_module(nn.Conv2d(4 * channels, channels, 1)),
            )
        else:
            self.ffn = None
    ...
    def attention(self, q, k, v, mask=None):
        ...
        weight = mx.softmax(weight.float(), axis=-1).type(weight.dtype)
...
class SelfAttention1D(nn.Module):
    def __init__(
        ...
        self.proj_out = mlx_zero_module(nn.Linear(channels, channels))
        if use_attention_ffn:
            self.ffn = nn.Sequential(
                nn.LayerNorm(channels),
                nn.Linear(channels, 4 * channels),
                nn.GELU(),
                mlx_zero_module(nn.Linear(4 * channels, channels)),
            )
        else:
            self.ffn = None
        if pos_emb:
...
class TemporalAttentionBlock(nn.Module):
    def __init__(
        ...
        self.mlp = MLX_MLP(channels, multiplier=4)
        self.down = down
        if down:
    ...
    def forward(self, x, temb):
        ...
        if self.down:
            x = self.up_conv(nn.Upsample(x,scale_factor=2, mode='linear'))
...
class MLX_MLP(nn.Module):
    def __init__(self, channels, multiplier=4):
        ...
        self.main = nn.Sequential(
            ...
            nn.GELU(),
            mlx_zero_module(nn.Linear(multiplier * channels, channels)),
        )
...
class SelfAttention1DBlock(nn.Module):
    def __init__(self, channels, num_heads=8, num_head_channels=-1, mlp_multiplier=4):
        ...
        self.mlp = MLX_MLP(channels, mlp_multiplier)
    ...
    def forward(
        ...
        if self.downsample_output or self.upsample_output:
            ...
            if self.upsample_output:
                x = nn.Upsample(x.type(mx.float32), scale_factor=2).type(x.dtype)
            x = self.resample(x)
            if self.temporal and (not self.temporal_spatial_ds):
...
class UNet(nn.Module):
    def __init__(self, input_channels, output_channels, config):
        ...
        if config.micro_conditioning is not None:
            ...
            for condition in self.conditions:
                cond_layers[condition] = nn.quantize(
                    [
    ...
    def print_size(self, target_image_size=64):
        summary(
            ...
            dtypes=[mx.float16, mx.float16, mx.float16, mx.float16],
    ...
    def create_temporal_embedding(self, times, ff_layers=None):
        ...
        temb = mx.concat([mx.sin(temb), mx.cos(temb)], dim=1)
        if temb.shape[1] % 2 == 1:
            # zero pad
            temb = mx.concat([temb, mx.zeros(times.shape[0], 1)], dim=1)
